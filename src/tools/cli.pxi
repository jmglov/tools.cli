...
(defn- whitespace? [c]
  (contains? (set s/whitespace) c))

(defn- partition
  ([n coll] (pixie.stdlib/partition n coll))
  ([n step coll] (pixie.stdlib/partition n step coll))
  ([n step pad coll]
   (let [parts (pixie.stdlib/partition n step coll)
         last-size (count (last parts))]
     (if (= n last-size)
       parts
       (concat (take (dec (count parts)) parts)
               [(concat (last parts) (take (- n last-size) pad))])))))

(defn- split
  "Splits string by splitter limited to the specified number of splits. Example:
  (split \"--foo=bar=baz=blah\" \"=\" 2)  ;;=> [\"--foo\" \"bar=baz=blah\"]"
  [s splitter limit]
  (let [splits (s/split s splitter)]
    (if (< (count splits) limit)
      splits
      (concat (take (dec limit) splits)
              (->> splits
                   (drop (dec limit))
                   (s/join splitter)
                   vector)))))
    ...
    (loop [opts []
      ...
      (if arg
        (cond
          ...
          (= "--" arg)
          (recur opts (into argv args) [])

          ;; Long options with assignment always passes optarg, required or not
          (and (s/starts-with? arg "--")
               (when-let [i (s/index-of arg "=")] (not (some whitespace? (s/substring arg 2 i)))))
          (recur (conj opts (into [:long-opt] (split arg "=" 2)))
                 argv args)

          ;; Long options, consumes args head if needed
          (s/starts-with? arg "--")
          (let [[optarg args] (if (contains? required-set arg)
                                [(first args) (rest args)]
                                [nil args])]
            (recur (conj opts (into [:long-opt arg] (if optarg [optarg] [])))
                   argv args))

          ;; Short options, expands clumped opts until an optarg is required
          (and (s/starts-with? arg "-") (>= (count arg) 2))
          (let [[os args] (loop [os [] [c & cs] (rest arg)]
                            (let [o (str \- c)]
                              (if (contains? required-set o)
                                (if (seq cs)
                                  ;; Get optarg from rest of arg
                                  [(conj os [:short-opt o (s/join cs)]) args]
                                  ;; Get optarg from head of args
                                  [(conj os [:short-opt o (first args)]) (rest args)])
                                (if (seq cs)
                                  (recur (conj os [:short-opt o]) cs)
                                  [(conj os [:short-opt o]) args]))))]
            (recur (into opts os) argv args))

          :else
          (if in-order
            (recur opts (into argv (cons arg args)) [])
            (recur opts (conj argv arg) args)))
        [opts argv]))))

(defn cli
  "THIS IS A LEGACY FUNCTION and has been removed. Use tools.cli/parse-opts instead."
  [args & specs]
  (println "THIS IS A LEGACY FUNCTION and has been removed. Use tools.cli/parse-opts instead."))
...
(defn- compile-spec [spec]
  (let [sopt-lopt-desc (take-while #(or (string? %) (nil? %)) spec)
    ...
    (merge {:id id
           ...
           (select-keys (dissoc spec-map :validate) spec-keys))))
...
(defn parse-opts
  ...
  parse-opts:
    ...
    :summary-fn   A function that receives the sequence of compiled option specs
                  (documented at #'tools.cli/compile-option-specs), and
